...
procedure Usage(missingArg)
    ...
    print "  printAllStructuralStats Optional. If true, print one CSV row for every element with structural statistics. Default: false.";
...
printAllStructuralStats := assigned printAllStructuralStats select (printAllStructuralStats eq "true" or printAllStructuralStats eq "True" or printAllStructuralStats eq "1") else false;
...
// -----------------------------------------------------------------------------
// Structural statistics for where the corrections live
// -----------------------------------------------------------------------------

function NodeSetString(I)
    S := Sort(Setseq(I));
    return #S eq 0 select "empty" else Join([IntegerToString(i) : i in S], "-");
end function;

function SupportNodes(w)
    return Seqset(Eltseq(w));
end function;

function CanonicalOrbitRep(w, aut)
    seq := [w, w^-1, aut(w), aut(w^-1)];
    Sort(~seq, func<x,y | CompareElts(x, y) >);
    return seq[1];
end function;

procedure AddToAssoc(~A, key, inc)
    ok, val := IsDefined(A, key);
    A[key] := ok select val + inc else inc;
end procedure;

procedure PrintStructuralSummary(W, C, pC, aut, type, char, targetLength, maxExamples, printAllStructuralStats)
    RF := RealField(12);
    allElts := Sort(Setseq(EnumerateCoxeterGroup(W : lengthBound := targetLength)));
    nTotal := #allElts;
    changed := [w : w in allElts | pC.w ne C.w];
    ...
    massByW := AssociativeArray(W);
    supportByW := AssociativeArray(W);
    monomialByW := AssociativeArray(W);
    depthByW := AssociativeArray(W);
    degreeMassByW := AssociativeArray(W);
    for s in correctionStats do
        massByW[s[1]] := s[9];
        supportByW[s[1]] := s[3];
        monomialByW[s[1]] := s[10];
        depthByW[s[1]] := s[7];
        degreeMassByW[s[1]] := s[14];
    end for;

    fullElts := [w : w in allElts | #SupportNodes(w) eq rank];
    inheritedElts := [w : w in allElts | #SupportNodes(w) lt rank];
    fullChanged := [w : w in changed | #SupportNodes(w) eq rank];
    inheritedChanged := [w : w in changed | #SupportNodes(w) lt rank];

    changedOrbitRepsSet := {CanonicalOrbitRep(w, aut) : w in changed};
    changedOrbitReps := Sort(Setseq(changedOrbitRepsSet));
    Sort(~changedOrbitReps, func<x,y | CompareElts(x, y) >);

    // Correction-summand depth distributions.
    depthSummands := AssociativeArray(Integers());
    depthAbsMass := AssociativeArray(Integers());
    depthMonomials := AssociativeArray(Integers());
    depthWeightedDegreeMass := AssociativeArray(Integers());

    depthDegreeAbsMass := AssociativeArray();
    depthDegreeMonomials := AssociativeArray();

    targetSupportFreq := AssociativeArray();
    sourceTargetSupportFreq := AssociativeArray();

    for w in changed do
        delta := CorrectionInCBasis(C, pC, w);
        for x in Support(delta) do
            f := Coefficient(delta, x);
            depth := #w - #x;
            AddToAssoc(~depthSummands, depth, 1);
            AddToAssoc(~depthAbsMass, depth, PolyAbsMass(f));
            AddToAssoc(~depthMonomials, depth, PolyMonomialCount(f));
            AddToAssoc(~depthWeightedDegreeMass, depth, PolyWeightedAbsDegreeMass(f));

            AddToAssoc(~targetSupportFreq, NodeSetString(SupportNodes(x)), 1);
            AddToAssoc(~sourceTargetSupportFreq, NodeSetString(SupportNodes(w)) cat "->" cat NodeSetString(SupportNodes(x)), 1);

            if f ne 0 then
                for deg in [Valuation(f)..Degree(f)] do
                    c := Coefficient(f, deg);
                    if c ne 0 then
                        key := <depth, Abs(deg)>;
                        AddToAssoc(~depthDegreeAbsMass, key, Abs(c));
                        AddToAssoc(~depthDegreeMonomials, key, 1);
                    end if;
                end for;
            end if;
        end for;
    ...
    printf "Maximum length considered: %o\n", maxLen;
    printf "Different from KL: %o of %o\n", nChanged, nTotal;
    printf "Correction basis: ordinary KL basis, Delta_w = pC(w)-C(w) expanded in C-basis.\n";
    printf "Structural convention: support_nodes is the set of simple reflections appearing in a reduced expression.\n";
    printf "Orbit convention: w is identified with w^-1, sigma(w), and sigma(w^-1), where sigma is the diagram automorphism used by the script.\n";

    print "";
    print "FULL SUPPORT SUMMARY";
    print "class,total,changed,percentage_changed,avg_abs_mass_at_one,avg_support_size,avg_monomial_complexity,avg_max_bruhat_depth,avg_weighted_abs_degree_mass";
    for label in ["full_support", "proper_parabolic"] do
        if label eq "full_support" then
            elts := fullElts;
            ch := fullChanged;
        else
            elts := inheritedElts;
        ...
        stats := [s : s in correctionStats | (label eq "full_support" and #SupportNodes(s[1]) eq rank) or (label eq "proper_parabolic" and #SupportNodes(s[1]) lt rank)];
        pct := #elts eq 0 select RF!0 else (RF!(100*#ch))/(RF!#elts);
        printf "%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
            label, #elts, #ch, pct, StatAverage(stats, 9), StatAverage(stats, 3), StatAverage(stats, 10), StatAverage(stats, 7), StatAverage(stats, 14);
    end for;

    print "";
    print "SYMMETRY ORBIT SUMMARY";
    print "changed_elements,changed_orbits,average_orbit_size";
    avgOrbitSize := #changedOrbitReps eq 0 select RF!0 else (RF!nChanged)/(RF!#changedOrbitReps);
    printf "%o,%o,%o\n", nChanged, #changedOrbitReps, avgOrbitSize;

    print "";
    print "SUPPORT PROFILE";
    print "support_nodes,support_size,total,changed,percentage_changed,avg_abs_mass_at_one,avg_support_size,avg_monomial_complexity,avg_max_bruhat_depth,avg_weighted_abs_degree_mass";
    supportKeys := Sort(Setseq({NodeSetString(SupportNodes(w)) : w in allElts}));
    for key in supportKeys do
        elts := [w : w in allElts | NodeSetString(SupportNodes(w)) eq key];
        ch := [w : w in changed | NodeSetString(SupportNodes(w)) eq key];
        stats := [s : s in correctionStats | NodeSetString(SupportNodes(s[1])) eq key];
        pct := #elts eq 0 select RF!0 else (RF!(100*#ch))/(RF!#elts);
        // Recover support size from a representative, avoiding string parsing.
        ksize := #elts eq 0 select 0 else #SupportNodes(elts[1]);
        printf "%o,%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
            key, ksize, #elts, #ch, pct, StatAverage(stats, 9), StatAverage(stats, 3), StatAverage(stats, 10), StatAverage(stats, 7), StatAverage(stats, 14);
    end for;

    print "";
    print "SUPPORT SIZE PROFILE";
    print "support_size,total,changed,percentage_changed,avg_abs_mass_at_one,avg_support_size,avg_monomial_complexity,avg_max_bruhat_depth,avg_weighted_abs_degree_mass";
    for k in [0..rank] do
        elts := [w : w in allElts | #SupportNodes(w) eq k];
        ch := [w : w in changed | #SupportNodes(w) eq k];
        stats := [s : s in correctionStats | #SupportNodes(s[1]) eq k];
        pct := #elts eq 0 select RF!0 else (RF!(100*#ch))/(RF!#elts);
        printf "%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
            k, #elts, #ch, pct, StatAverage(stats, 9), StatAverage(stats, 3), StatAverage(stats, 10), StatAverage(stats, 7), StatAverage(stats, 14);
    end for;

    print "";
    print "DESCENT PROFILE";
    print "left_descents,right_descents,total,changed,percentage_changed,avg_abs_mass_at_one,avg_support_size,avg_monomial_complexity,avg_max_bruhat_depth,avg_weighted_abs_degree_mass";
    for l in [0..rank] do
        for r in [0..rank] do
            elts := [w : w in allElts | #LeftDescentSet(W, w) eq l and #RightDescentSet(W, w) eq r];
            if #elts eq 0 then
                continue;
            end if;
            ch := [w : w in changed | #LeftDescentSet(W, w) eq l and #RightDescentSet(W, w) eq r];
            stats := [s : s in correctionStats | #LeftDescentSet(W, s[1]) eq l and #RightDescentSet(W, s[1]) eq r];
            pct := (RF!(100*#ch))/(RF!#elts);
            printf "%o,%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
                l, r, #elts, #ch, pct, StatAverage(stats, 9), StatAverage(stats, 3), StatAverage(stats, 10), StatAverage(stats, 7), StatAverage(stats, 14);
        end for;
    end for;

    print "";
    print "BRUHAT DEPTH DISTRIBUTION OF CORRECTION SUMMANDS";
    print "depth,summands,total_abs_mass,total_monomials,total_weighted_abs_degree_mass";
    depthKeys := Sort(Setseq(Keys(depthSummands)));
    for d in depthKeys do
        printf "%o,%o,%o,%o,%o\n", d, depthSummands[d], depthAbsMass[d], depthMonomials[d], depthWeightedDegreeMass[d];
    end for;

    print "";
    print "BRUHAT DEPTH BY ABS DEGREE DISTRIBUTION";
    print "depth,abs_degree,total_abs_mass,monomials";
    ddKeys := Setseq(Keys(depthDegreeAbsMass));
    Sort(~ddKeys, func<a,b | (a[1] ne b[1]) select (a[1] - b[1]) else (a[2] - b[2]) >);
    for key in ddKeys do
        printf "%o,%o,%o,%o\n", key[1], key[2], depthDegreeAbsMass[key], depthDegreeMonomials[key];
    end for;

    print "";
    print "CORRECTION TARGET SUPPORT PROFILE";
    print "target_support_nodes,summands";
    targetRows := StringCountRows(targetSupportFreq);
    for row in targetRows do
        printf "%o,%o\n", row[1], row[2];
    end for;

    print "";
    print "SOURCE TO TARGET SUPPORT PROFILE";
    print "source_to_target_support_nodes,summands";
    stRows := StringCountRows(sourceTargetSupportFreq);
    nST := Min(maxExamples, #stRows);
    for i in [1..nST] do
        row := stRows[i];
        printf "%o,%o\n", row[1], row[2];
    end for;
    ...
    print "";
    print "SYMMETRY-REDUCED FIRST WITNESSES";
    print "orbit_rep,length,support_nodes,support_size,left_descents,right_descents,abs_mass_at_one,monomial_complexity,max_bruhat_depth,weighted_abs_degree_mass,graded_mass_polynomial,pC(w)-C(w) in C-basis";
    repStats := [];
    for rep in changedOrbitReps do
        // The representative might not itself be changed if the canonical representative sits outside the changed subset in exotic cases.
        // In our setting it should be changed by symmetry; still choose the first changed element in its orbit for safety.
        orbit := [rep, rep^-1, aut(rep), aut(rep^-1)];
        orbitChanged := [u : u in orbit | pC.u ne C.u];
        chosen := #orbitChanged eq 0 select rep else orbitChanged[1];
        Append(~repStats, CorrectionStats(W, C, pC, chosen));
    end for;
    Sort(~repStats, func<a,b | (a[2] ne b[2]) select (a[2] - b[2]) else CompareElts(a[1], b[1]) >);
    nPrint := Min(maxExamples, #repStats);
    for i in [1..nPrint] do
        s := repStats[i];
        w := s[1];
        printf "%o,%o,%o,%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
            FmtElt(w), #w, NodeSetString(SupportNodes(w)), #SupportNodes(w), #LeftDescentSet(W, w), #RightDescentSet(W, w),
            s[9], s[10], s[7], s[14], s[8], CorrectionInCBasis(C, pC, w);
    end for;

    if printAllStructuralStats then
        print "";
        print "ALL STRUCTURAL STATS";
        print "word,length,changed,support_nodes,support_size,full_support,left_descents,right_descents,orbit_rep,abs_mass_at_one,monomial_complexity,max_bruhat_depth,weighted_abs_degree_mass,graded_mass_polynomial";
        for w in allElts do
            isChanged := pC.w ne C.w;
            if isChanged then
                s := CorrectionStats(W, C, pC, w);
                printf "%o,%o,1,%o,%o,%o,%o,%o,%o,%o,%o,%o,%o,%o\n",
                    FmtElt(w), #w, NodeSetString(SupportNodes(w)), #SupportNodes(w), (#SupportNodes(w) eq rank select 1 else 0),
                    #LeftDescentSet(W, w), #RightDescentSet(W, w), FmtElt(CanonicalOrbitRep(w, aut)), s[9], s[10], s[7], s[14], s[8];
            else
                printf "%o,%o,0,%o,%o,%o,%o,%o,%o,0,0,0,0,0\n",
                    FmtElt(w), #w, NodeSetString(SupportNodes(w)), #SupportNodes(w), (#SupportNodes(w) eq rank select 1 else 0),
                    #LeftDescentSet(W, w), #RightDescentSet(W, w), FmtElt(CanonicalOrbitRep(w, aut));
            end if;
        end for;
    end if;

    print "============================================================";
    print "END PKL STRUCTURAL SUMMARY";
    print "============================================================";
end procedure;
...
PrintStructuralSummary(W, C, pC, aut, type, char, targetLength, maxExamples, printAllStructuralStats);
